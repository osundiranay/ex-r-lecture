...
# change name of column&row in "Free-Throws" Matrix:
...
#
# <put your code here>
## Init "Free-Throws-Attempts" Matrix:
...
# Create Function to visualize our data 
data_visualization <- function(raw_matrix, rows=1:10){
  subset_matrix = raw_matrix[rows,, drop=F]
  matplot(t(subset_matrix), type="b", pch=15:18, col=c(1:4,6))
  legend("bottomleft", inset=0.001, legend=Players[rows], col=c(1:4,6), pch=15:18, horiz=F)
}

# Plot: Free Throw Attempts per game
data_visualization(FreeThrowsAttempts/Games)

# plot: Accuracy of Free Throws
data_visualization(round(FreeThrows/FreeThrowsAttempts,digits=1))
